...
**重要：TrendRadar 不是实时更新系统**

在开始集成前，了解 TrendRadar 的数据更新机制很重要：

### 更新方式
- **定时爬虫**：通过 GitHub Actions 定时运行（默认每小时一次）
- **非实时**：不会自动刷新浏览器页面，需要用户手动刷新
- **自动推送**：爬虫完成后自动更新到 GitHub Pages

### 更新频率
- 默认：每小时整点运行（如 8:00、9:00、10:00...）
- 可自定义：修改 `.github/workflows/crawler.yml` 中的 cron 表达式
...
**自动更新（推荐）：**
- 等待下一个整点，爬虫自动运行
- 查看 Actions 标签确认工作流完成
- 刷新浏览器页面

**手动触发：**
1. 进入 Actions → Hot News Crawler
2. 点击 "Run workflow"
3. 等待 2-3 分钟完成
4. 刷新页面查看新数据

### 查看数据时间
页面头部显示的"生成时间"是数据最后更新时间，不是当前时刻。
